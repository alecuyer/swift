...
class TestDiskFileRouter(unittest.TestCase):
    ...
    def test_invalid_policy_config(self):
        ...
        bad_policy = StoragePolicy(0, name='zero', is_default=True,
                                   diskfile_uri='erasure_coding.fs')

        with patch_policies([bad_policy]):
            with self.assertRaises(PolicyError) as cm:
                diskfile.DiskFileRouter({}, debug_logger())
        self.assertIn('Invalid diskfile erasure_coding.fs', str(cm.exception))

        bad_policy = ECStoragePolicy(0, name='one', is_default=True,
                                     ec_type=DEFAULT_TEST_EC_TYPE,
                                     ec_ndata=10, ec_nparity=4,
                                     diskfile_uri='replication.fs')

        with patch_policies([bad_policy]):
            with self.assertRaises(PolicyError) as cm:
                diskfile.DiskFileRouter({}, debug_logger())
        self.assertIn('Invalid diskfile replication.fs', str(cm.exception))

        bad_policy = StoragePolicy(0, name='zero', is_default=True,
                                   diskfile_uri='thin_air.fs')

        with patch_policies([bad_policy]):
            with self.assertRaises(PolicyError) as cm:
                diskfile.DiskFileRouter({}, debug_logger())
        self.assertIn('Unable to load diskfile thin_air.fs', str(cm.exception))
...
class TestDiskFileManager(DiskFileManagerMixin, unittest.TestCase):
    ...
    def test_check_policy(self):
        diskfile.DiskFileManager.check_policy(POLICIES[0])
        with self.assertRaises(ValueError) as cm:
            diskfile.DiskFileManager.check_policy(POLICIES[1])
        self.assertEqual("Invalid policy_type: erasure_coding",
                         str(cm.exception))
...
class TestECDiskFileManager(DiskFileManagerMixin, unittest.TestCase):
    ...
    @patch_policies(test_policies)
    def test_check_policy(self):
        diskfile.ECDiskFileManager.check_policy(POLICIES[1])
        with self.assertRaises(ValueError) as cm:
            diskfile.ECDiskFileManager.check_policy(POLICIES[0])
        self.assertEqual("Invalid policy_type: replication",
                         str(cm.exception))
