...
def register_volume(socket_path, partition, volume_type, volume_index,
    ...
    volume = pb.RegisterVolumeRequest(partition=int(partition), type=int(volume_type),
...
def register_object(socket_path, name, volume_index, offset, next_offset,
    ...
    register a vfile
    """
    obj = pb.RegisterObjectRequest(name=str(name), volume_index=int(volume_index),
                           offset=int(offset),
...
def quarantine_object(socket_path, name, repair_tool=False):
    objname = pb.QuarantineObjectRequest(name=str(name), repair_tool=repair_tool)
    conn = UnixHTTPConnection(socket_path)
...
def unquarantine_object(socket_path, name, repair_tool=False):
    objname = pb.UnquarantineObjectRequest(name=str(name), repair_tool=repair_tool)
    conn = UnixHTTPConnection(socket_path)
...
def _list_objects_by_volume(socket_path, volume_index, quarantined, page_token,
    ...
    """
    req_args = pb.LoadObjectsByVolumeRequest(index=volume_index, quarantined=quarantined,
...
def list_quarantined_ohash(socket_path, prefix, repair_tool=False):
    ...
    prefix = pb.ListQuarantinedOHashRequest(prefix=str(prefix), repair_tool=repair_tool)
    conn = UnixHTTPConnection(socket_path)
...
def list_prefix(socket_path, prefix, repair_tool=False):
    len_prefix = len(prefix)
    prefix = str(prefix)
    prefix = pb.LoadObjectsByPrefixRequest(prefix=prefix, repair_tool=repair_tool)
    conn = UnixHTTPConnection(socket_path)
