...
from header import ObjectHeader, VolumeHeader, ALIGNMENT, read_volume_header, \
    HeaderException, STATE_OBJ_QUARANTINED, STATE_OBJ_FILE, write_header, \
    read_header
...
from vfile_utils import SwiftPathInfo, get_volume_index, get_volume_type, \
    next_aligned_offset, punch_hole
...
class VFileReader(object):
    ...
    def _get_vfile(cls, name, volume_dir, socket_path, logger,
        ...
        # get the object
        try:
            obj = rpc.get_object(socket_path, name, repair_tool=repair_tool)
...
def create_writable_volume(socket_path, partition, volume_type, volume_dir,
    ...
    # Write volume header
    os.write(volume_file, vol_header.pack())
...
# TODO add function to list content of a quarantined object
def list_quarantine(socket_path):
    """
    Lists all quarantined object hashes for the disk/policy
    :param socket_path: socket path to the KV
    :return: a list of quarantined object hashes
    """
    return rpc.list_quarantined_ohashes(socket_path)
...
def set_header_state(socket_path, name, quarantine):
    ...
    with open(volume_filepath, 'r+b') as fp:
        ...
        if quarantine:
            ...
            header.state = STATE_OBJ_FILE
        write_header(header, fp, obj.offset)


def quarantine_ohash(dirpath):
    """
...
def rename_vfile(filepath, newfilepath, logger):
    ...
    vf_reader = VFileReader._get_vfile(full_name, si.volume_dir, si.socket_path,
                                       logger)
