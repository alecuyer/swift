...
def get_voldir_from_quarantine_path(path):
    """
    Given a path such as /<device>/quarantined/<policy>, or below,
    return the volume directory, or raises ValueError
    :param path: quarantine path
    :return: volume directory, or None if it cannot be determined
    """
    socket_path = os.path.join(_get_losf_dir_from_quarantine_path(path),
                               'rpc.socket')
    return socket_path
