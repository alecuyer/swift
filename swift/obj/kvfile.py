...
from swift.common.utils import fdatasync, \
    ...
    get_md5_socket, o_tmpfile_supported
...
def quarantine_vrenamer(device_path, corrupted_file_path):
    ...
    # rename key in KV
    return vfile.quarantine_ohash(dirname(corrupted_file_path))
...
class BaseKVFile(BaseDiskFile):
    ...
    def _verify_data_file(self, data_file, data_vfr, current_time):
        ...
        obj_size = data_vfr._header.data_size
    ...
    def create(self, size=None, extension=None):
        ...
        try:
            tmppath = None
            vfile_writer = vfile.VFileWriter.create(self._datadir, size,
                                                    self._manager.vfile_conf,
                                                    self._logger,
                                                    extension=extension)
        ...
        dfw = None
        try:
            dfw = self.writer_cls(self._name, self._datadir, vfile_writer,
                                  tmppath, bytes_per_sync=self._bytes_per_sync,
                                  diskfile=self)
            yield dfw
        finally:
            try:
                os.close(vfile_writer.fd)
                os.close(vfile_writer.lock_fd)
            except OSError:
                pass
...
class BaseKVFileWriter(BaseDiskFileWriter):
    def __init__(self, name, datadir, vfile_writer, tmppath, bytes_per_sync,
                 diskfile):
        # Parameter tracking
        self._name = name
        self._datadir = datadir
        self._vfile_writer = vfile_writer
