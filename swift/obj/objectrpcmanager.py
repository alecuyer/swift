...
class ObjectRpcManager(Daemon):
    def __init__(self, conf, logger=None):
        ...
        self.wait_for_mount = config_true_value(conf.get('wait_for_mount',
                                                         'false'))

        # check if the path to LOSF binary and volume checker exist
        if not os.path.exists(self.volcheck):
            raise AttributeError("Invalid or missing volcheck in your config file")
        if not os.path.exists(self.losf_bin):
            raise AttributeError("Invalid or missing losf_bin in your config file")
    ...
    def get_kv_disks(self):
        ...
        for policy, devs in policy2devices.items():
            if policy.diskfile_uri.endswith('.kv'):
    ...
    def run_forever(self, policy_idx=None, disk_path=None, socket_path=None,
        ...
        if statecheck:
            volcheck_args = [self.volcheck, '--disk_path', str(disk_path), '--policy_idx', str(policy_idx), '--keepuser', '--repair']
            ...
            # TODO wait for mount must be an option
            losf_args = ['swift-losf-rpc', '-diskPath', str(disk_path), '-policyIdx', str(policy_idx), '-waitForMount=false']
