...
@rpc
def list_volumes(socket_path, partition, type, get_all_volumes=False,
                 repair_tool=False):
    list_info = fmgr_pb2.ListVolumesInfo(partition=int(partition), type=type,
                                         get_all_volumes=get_all_volumes,
                                         repair_tool=repair_tool)
...
def get_volume_stats(socket_path):
    e = fmgr_pb2.Empty()
    response = connections.connections[socket_path].stub.GetVolumeStats(e)
