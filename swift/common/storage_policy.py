...
class BaseStoragePolicy(object):
    ...
    def __init__(self, idx, name='', is_default=False, is_deprecated=False,
                 ...
                 diskfile_uri='egg:swift#replication.fs'):
        ...
        self.diskfile_uri = diskfile_uri
    ...
    def get_diskfile_manager(self, *args, **kwargs):
        ...
        """
        try:
            dfm_cls = load_pkg_resource('swift.diskfile', self.diskfile_uri)
        except ImportError as err:
            raise PolicyError('Unable to load diskfile %s for policy %s: %s' %
                              (self.diskfile_uri, self.name, err))
        try:
            dfm_cls.check_policy(self)
        except ValueError as err:
            raise PolicyError('Invalid diskfile %s for policy %s: %s' %
                              (self.diskfile_uri, self.name, err))
...
class ECStoragePolicy(BaseStoragePolicy):
    ...
    def __init__(self, idx, name='', aliases='', is_default=False,
                 ...
                 diskfile_uri='egg:swift#erasure_coding.fs',
