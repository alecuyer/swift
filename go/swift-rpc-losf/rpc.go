...
func (s *server) ListVolumes(ctx context.Context, in *pb.ListVolumesInfo) (*pb.Volumes, error) {
	reqlog := log.WithFields(logrus.Fields{"Function": "ListVolumes", "Partition": in.Partition, "GetAllVolumes": in.GetAllVolumes, "Type": in.Type})
	reqlog.Debug("RPC Call")
	...
	for it.SeekToFirst(); it.Valid(); it.Next() {
		...
		if in.GetAllVolumes || (uint32(partition) == in.Partition && pb.VolumeType(dfType) == in.Type) {
...
func (s *server) GetVolumeStats(ctx context.Context, in *pb.Empty) (*pb.VolumeStats, error) {
	...
	volumeStats := &pb.VolumeStats{}
	volumeStats.Volume = make(map[uint32]*pb.VolumeStat)

	// Iterate over volumes
	itv := s.kv.NewIterator(volumePrefix)
	defer itv.Close()

	for itv.SeekToFirst(); itv.Valid(); itv.Next() {
		idx, err := DecodeVolumeKey(itv.Key())
		if err != nil {
			reqlog.Errorf("failed to decode volume key: %v", err)
			s.statsd_c.Increment("list_volumes.fail")
			return nil, status.Errorf(codes.Internal, "unable to decode volume value")
		...
		volumeStats.Volume[idx] = &pb.VolumeStat{}
	}
	...
	for it.SeekToFirst(); it.Valid(); it.Next() {
		...
		if _, ok := volumeStats.Volume[volumeIndex]; ok {
			volumeStats.Volume[volumeIndex].ObjectCount += 1
		} else {
			volumeStats.Volume[volumeIndex] = &pb.VolumeStat{}
			volumeStats.Volume[volumeIndex].ObjectCount = 1
		}
	}

	// Iterate over all quarantined objects
	itq := s.kv.NewIterator(quarantinePrefix)
	defer itq.Close()

	for itq.SeekToFirst(); itq.Valid(); itq.Next() {
		volumeIndex, _, err := DecodeObjectValue(itq.Value())
		if err != nil {
			reqlog.Errorf("failed to decode object value: %v", err)
			s.statsd_c.Increment("load_objects_by_volume.fail")
			return nil, status.Errorf(codes.Internal, "unable to read object")
		...
		if _, ok := volumeStats.Volume[volumeIndex]; ok {
			volumeStats.Volume[volumeIndex].QuarantinedCount += 1
		} else {
			volumeStats.Volume[volumeIndex] = &pb.VolumeStat{}
			volumeStats.Volume[volumeIndex].QuarantinedCount = 1
		}
...
func (s *server) QuarantineDir(ctx context.Context, in *pb.ObjectPrefix) (*pb.DelObjectReply, error) {
	...
	for it.Seek(prefix); it.Valid() && len(prefix) <= len(it.Key()) && bytes.Equal(prefix, it.Key()[:len(prefix)]); it.Next() {
		...
		if err != nil {
			reqlog.Errorf("failed to Put new quarantined object entry: %v", err)
			s.statsd_c.Increment("quarantine_dir.fail")
...
func (s *server) LoadObjectsByVolume(in *pb.VolumeIndex, stream pb.FileMgr_LoadObjectsByVolumeServer) error {
	...
	for it.SeekToFirst(); it.Valid(); it.Next() {
		...
		if volumeIndex == in.Index {
			...
			if err != nil {
				reqlog.Errorf("failed to send streamed response: %v", err)
				s.statsd_c.Increment("load_objects_by_volume.fail")
...
func (s *server) ListQuarantinedOHashes(in *pb.Empty, stream pb.FileMgr_ListQuarantinedOHashesServer) error {
	...
	for it.SeekToFirst(); it.Valid(); it.Next() {
		...
		if !bytes.Equal(curKey[:32], lastOhash) {
			...
			if err != nil {
				reqlog.Errorf("failed to send streamed respone in ListQuarantinedOhashes: %v", err)
				s.statsd_c.Increment("list_quarantined_ohashes.fail")
