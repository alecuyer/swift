...
// Encodes a data file key.
func EncodeDataFileKey(index uint32) (val []byte) {
	buf := make([]byte, binary.MaxVarintLen32)

	n := binary.PutUvarint(buf, uint64(index))

	val = buf[:n]
	log.WithFields(logrus.Fields{"value": val}).Debug("encoded data file key")
	return
}

func DecodeDataFileKey(val []byte) (index uint32, err error) {
...
// dataFileType is an int32 to match the type generated by protobuf for enums
func EncodeDataFileValue(partition int64, dataFileType int32, nextOffset, usedSpace, state int64) (val []byte) {
	...
	n = binary.PutUvarint(buf[bufLen:], uint64(dataFileType))
	bufLen += n
	...
	val = buf[:bufLen]
	log.WithFields(logrus.Fields{"value": val}).Debug("encoded data file value")
	return
}

func DecodeDataFileValue(val []byte) (partition int64, dataFileType int32, nextOffset, usedSpace, state int64, err error) {
	position := 0
	...
	dataFileType64, n := binary.Uvarint(val[position:])
	if n <= 0 {
	...
	dataFileType = int32(dataFileType64)
...
func EncodeObjectValue(dataFileIndex uint32, offset uint64) (val []byte) {
	buf := make([]byte, binary.MaxVarintLen64*2)
	bufLen := 0

	n := binary.PutUvarint(buf, uint64(dataFileIndex))
	bufLen += n
...
func DecodeObjectValue(val []byte) (dataFileIndex uint32, offset uint64, err error) {
	log.WithFields(logrus.Fields{"value": val}).Debug("Decode object value")
	dataFileIndex64, n := binary.Uvarint(val)
	if n <= 0 {
		log.WithFields(logrus.Fields{"index": n}).Debug("failed to decode dataFileIndex")
		err = errors.New("failed to decode dataFileIndex")
	}
	...
	dataFileIndex = uint32(dataFileIndex64)
